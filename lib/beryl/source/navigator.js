(function(){
  ...
  enyo.kind({
      ...
      {name: "applicationContainer", classes: "row", components: [
        ...
        {name: "workspace", classes: "col-lg-10 col-md-10 no-padding",
         components: [
           ...
           {name: "messageHolder", classes: "col-lg-10 col-md-10 no-padding clearfix", components: [
             {kind: 'onyx.MenuDecorator', name: 'chatUsersButton', ontap: "loadChatUsers", style: 'float: right', components: [
               {content: "Users (#)"},
               {name: 'chatUserList', kind: "onyx.TitledMenu", menuTitle: "Online Users", ontap: "chatUserSelected", components: [
               ]}
             ]}
           ]}
         ]
    ...
    chatUserSelected: function(inSender, inEvent) {
      var action = inEvent.originator;
      this.generateNewChatBox(action);
      return true; // stop propagation to loadChatUsers
    },
    generateNewChatBox : function(action){
      var nameOfChat = action.value + "Chat",
          existingChats = [];

      // Gather all existingChats
      for(var i = 0; i < this.$.messageHolder.children.length; i++){
        existingChats.push(this.$.messageHolder.children[i].name);
      }

      // if chat does not exist then create it
      if(!(existingChats.indexOf(nameOfChat) != -1)){
        this.$.messageHolder.createComponent(
          {kind: 'onyx.MenuDecorator', name: nameOfChat, ontap: "", style: 'float: right', components: [
            {content: action.value},
            {name: action.value + "ChatBox", kind: "onyx.ChatBox", chatTitle: action.value, ontap: "", style: 'right: 0;', components: [
            ]}
          ]}
        );
        ...
        document.querySelectorAll("#" + this.$.messageHolder.children.splice(-1)[0].id + " button")[0].click();

        // then focus the input
        // do this becuase dealing with nodeLists are a headache
        var inputArr = [],
            nl = document.querySelectorAll('.chat-box-input-holder input');
        for(var i = nl.length; i--; inputArr.unshift(nl[i]));
