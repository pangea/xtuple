  ...
  enyo.kind({
    ...
    components: [
      {name: "applicationHeader", classes: "row", components: [
	    ...
	    {kind: "onyx.Input", name: "searchInput", classes: "search-input", placeholder: "Search...", onchange:"searchChanged"}
	  ]} 
        ]},
        {kind: 'onyx.MenuDecorator', name: 'userMenuButton', classes: "col-lg-1 col-md-1", onSelect: "userItemSelected", components: [
          {content: "User Menu", classes: "btn-link user-menu-button"},
      ...
      {name: "applicationActionHolder", classes: "row", components: [
        {name: "navigationMenu", classes: "col-lg-2 col-md-2", components: [
          {kind: "onyx.PickerDecorator", onSelect: "modulePickerChanged", components: [
            {classes: "btn-link module-picker"}, // onyx.PickerButton
            {name: "modulePicker", kind: "onyx.Picker", style: "max-height: auto"}
          ]}
        ]},
      ...
      {name: "applicationContainer", classes: "row", components: [
        ...
        {name: "workspace", classes: "col-lg-10 col-md-10 no-padding",
         ...
         // TODO: in the future, this should probably display some sort of verification
         modelSaved : function() { console.log('save successful'); }
        }
      ]}
    ],
    create: function() {
    ...
    */
    ...
    newRecord: function (inSender, inEvent) {
      var list = this.$.contentPanels.getActive(),
        ...
        listIsWorkspace = list instanceof XV.Workspace,
      ...
      } else if(listIsWorkspace) {
        if(list.defaultAttributes) {
          defaults = list.defaultAttributes();
        }
        list.newRecord(defaults);
      }
    ...
    saveWorkspace: function() {
      console.log( this.$.contentPanels );
      console.log( this.$.contentPanels.getActive() );
      console.log( this.$.contentPanels.getActive().save() );
    ...
    */
    ...
    setContentPanel: function (index, options) {
      ...
      // }
      if (!isWorkspace && panel.fetch && !this.fetched[panelIndex]) {
        this.fetch();
