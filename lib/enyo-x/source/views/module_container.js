...
(function () {
  ...
  enyo.kind(
    ...
    addWorkspace: function (inSender, inEvent, workspaceContainerKind) {
      ...
      workspaceContainerKind = workspaceContainerKind || "beryl.WorkspaceContainer"; // default
    ...
    */
    ...
    insertUserMenuItem: function(module, items) {
      var existingItems = this.$.navigator.$.userMenu.components;

      _.each(items, function (item) {
        item.module = module;
        existingItems.splice(0, 0, item);
      });
    ...
    notifyTap: function (inSender, inEvent) {
      ...
      if (typeof this._notifyCallback === 'function') {
        ...
        this._notifyCallback(callbackObj);
      } else {
        this.$.notifyPopup.hide();
      }
    },
    ...
    _setUserMenu: function() {
      var items = this.$.navigator.$.userMenu.components;
      this.$.navigator.setUserMenu(items);
    },
